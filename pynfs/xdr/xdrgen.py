...
def run(infile, filters=True, pass_attrs=True, debug=False, output_directory=''):
    ...
    comment_string = "# Generated by rpcgen.py from %s on %s\n" % \
                     (infile, time.asctime())
    const_fd = open(os.path.join(output_directory, constants_file + ".py"), "w")
    const_fd.write(comment_string)
    type_fd = open(os.path.join(output_directory, types_file + ".py"), "w")
    type_fd.write(comment_string)
    type_fd.write("import sys,os\nfrom . import %s as const\n" % constants_file)
    pack_fd = open(os.path.join(output_directory, packer_file + ".py"), "w")
